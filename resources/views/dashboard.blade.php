...
@section('content')
    <div class="card">
        ...
        <div class="card-subtitle">
            You are logged in as <strong>{{ Auth::user()->email }}</strong>. Use the quick links and metrics below to
        ...
        <div class="card-grid">
            <div class="metric-card">
                ...
                <div class="metric-value">{{ \App\Models\Project::count() }}</div>
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{{ \App\Models\Unit::count() }}</div>
                <div class="metric-trend">Track availability and pricing.</div>
                <div style="margin-top:10px;">
                    <a href="{{ route('units.index') }}" class="btn btn-primary">View Units</a>
                </div>
            </div>

            <div class="metric-card">
                <div class="metric-label">Active Bookings</div>
                <div class="metric-value">{{ \App\Models\Booking::where('status', 'booked')->count() }}</div>
            ...
            <div class="metric-card">
                ...
                <div class="metric-value">{{ \App\Models\Reminder::where('status', 'pending')->count() }}</div>
                <div class="metric-trend">Upcoming payment follow-ups.</div>
                <div style="margin-top:10px;">
                    <a href="{{ route('reminders.index') }}" class="btn btn-primary">View Reminders</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:10px;">
            ...
            <a href="{{ route('installment-plans.create') }}" class="btn btn-primary">New Installment Plan</a>
...
@section('scripts')
    ...
    <script>
        (function () {
            ...
            const monthLabels = {!! json_encode($monthLabels->values()) !!};
            const bookingSeries = {!! json_encode($bookingSeries->values()) !!};
            const paymentSeries = {!! json_encode($paymentSeries->values()) !!};
            const unitStatusLabels = {!! json_encode($unitStatusLabels->values()) !!};
            const unitStatusSeries = {!! json_encode($unitStatusSeries->values()) !!};
